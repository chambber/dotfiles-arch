...
function jira() {
  ...
  if [[ $action == "new" ]]; then
    ...
    wslview "${jira_url}/secure/CreateIssue!default.jspa"
  elif [[ "$action" == "assigned" || "$action" == "reported" ]]; then
    _jira_query ${@:-$action}
  elif [[ "$action" == "mine" ]]; then
    echo "Opening my issues"
    wslview "${jira_url}/issues/?filter=-1"
  elif [[ "$action" == "dashboard" ]]; then
    echo "Opening dashboard"
    if [[ "$JIRA_RAPID_BOARD" == "true" ]]; then
      _jira_rapid_board ${@}
    else
      wslview "${jira_url}/secure/Dashboard.jspa"
    fi
  elif [[ "$action" == "tempo" ]]; then
    echo "Opening tempo"
    if [[ -n "$JIRA_TEMPO_PATH" ]]; then
      wslview "${jira_url}${JIRA_TEMPO_PATH}"
    else
      wslview "${jira_url}/secure/Tempo.jspa"
    fi
    ...
    wslview "${jira_url}/browse/${issue}${url_fragment}"
  fi
}
...
function _jira_rapid_board() {
  ...
  if [[ -z $rapid_view ]]; then
    wslview "${jira_url}/secure/RapidBoard.jspa"
  else
    wslview "${jira_url}/secure/RapidBoard.jspa?rapidView=$rapid_view"
  fi
}
...
function _jira_query() {
  ...
  wslview "${jira_url}/secure/IssueNavigator.jspa?reset=true&jqlQuery=${query}"
}
